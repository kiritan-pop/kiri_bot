...
def build_transformer(
    ...
    output = decoder(
        ...
        source_mask=input_attention_mask,
    )

    model = tf.keras.Model(
        inputs=[input_ids, input_attention_mask, target_ids], outputs=[output])
...
class Decoder(tf.keras.models.Model):
    ...
    def __init__(
        ...
        for _ in range(hopping_num):
            ...
            ffn_layer = FeedForwardNetwork(
                hidden_dim, dropout_rate, name='ffn')
            self.attention_block_list.append([
                ResidualNormalizationWrapper(
                    self_attention_layer, dropout_rate, name='self_attention_wrapper'),
                ResidualNormalizationWrapper(
                    enc_dec_attention_layer, dropout_rate, name='enc_dec_attention_wrapper'),
                ResidualNormalizationWrapper(
        ...
        self.output_dense_layer = tf.keras.layers.Dense(
            vocab_size, use_bias=False, activation='softmax')
    ...
    def call(
        ...
        enc_attention_mask = self._create_enc_attention_mask(
            attention_mask=source_mask)
        if target_mask:
            dec_self_attention_mask = self._create_dec_self_attention_mask(
        ...
        for i, layers in enumerate(self.attention_block_list):
            self_attention_layer, enc_dec_attention_layer, ffn_layer = tuple(
                layers)
            with tf.name_scope(f'hopping_{i}'):
                query = self_attention_layer(
                    query, attention_mask=dec_self_attention_mask, training=training)
    ...
    def _create_enc_attention_mask(self, attention_mask: tf.Tensor):
        if attention_mask is not None:
            ...
            # [batch_size, m_length]
            pad_array = tf.equal(attention_mask, self.pad_id)
